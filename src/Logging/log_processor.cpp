...
Logging::LogProcessor::LogProcessor(std::atomic<size_t> *active_processors, std::condition_variable *log_cv,
                                    std::mutex *log_cv_mutex)
    : m_active_processors(active_processors), m_log_cv(log_cv), m_log_cv_mutex(log_cv_mutex)
{
...
void Logging::LogProcessor::run()
{
    ...
    while (m_should_run)
    {
...

        {
            ...
            there basically isn't any time for Processor to acquire a lock on the mutex.
            */
